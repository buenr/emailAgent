...
def list_message_classifications_by_run(
    conn: Any, run_log_id: int
) -> List[Dict[str, Any]]:
    """Return all message_classification rows for a given run_log entry."""
    if _demo(conn):
        return demo_db.list_message_classifications_by_run(run_log_id)
    cur = _execute(
        conn,
        "SELECT id, run_log_id, email_id, subject, sender, category, received_at, created_at "
        "FROM message_classification WHERE run_log_id = ? ORDER BY id",
        (run_log_id,),
    )
    rows = _fetchall_dicts(cur)
    for r in rows:
...
def list_message_classifications_by_inbox(
    ...
    base = (
        ...
        "mc.category, mc.received_at, mc.created_at "
        "FROM message_classification mc "
        "JOIN run_log rl ON rl.id = mc.run_log_id "
        "WHERE rl.inbox_id = ?"
    )
    count_base = (
        "SELECT COUNT(*) AS n FROM message_classification mc "
        "JOIN run_log rl ON rl.id = mc.run_log_id "
        "WHERE rl.inbox_id = ?"
    )
    params: list[Any] = [inbox_id]
    if category is not None and category.strip():
        clause = " AND mc.category = ?"
        base += clause
        count_base += clause
        params.append(category.strip())
    if since is not None and since.strip():
        clause = " AND mc.created_at >= ?"
        base += clause
        count_base += clause
